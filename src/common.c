...
* Return Value : none
********************************************************************************/
void Port_Set( UCHAR Port, UCHAR TermNum, UCHAR Val )//3,0,0 //3,1,0 //3,2,0
{
	volatile UCHAR *ucp_port;
	UCHAR	ucport;
	UCHAR	ucset = Val; //0 //0 //0

	ucp_port = &P0;
	ucp_port += Port;
	ucport = *ucp_port;//ucport=1 ucport=0 //ucport=1 ucport=0 //ucport=1 ucport=0
	ucset = 1;
	ucset <<= TermNum;//ucset=1  //ucset=2 //ucset=4
	if (Val == HIGH){//low
		ucport |= ucset;//ucport=1(0001),ucset=1(0001) = 1 ‘S‚Ä1
				
				//ucport=0(0000)=0 ‘S‚Ä0
	}else{
		ucport &= ~ucset;//ucport=1(0001),ucset=1(0001)¨~¨1110 = 0
	}                        //ucport=1(0001),ucset=2(0010)¨~¨1101 = 1
	                         //ucport=1(0001),ucset=4(0100)¨~¨0111 = 1
