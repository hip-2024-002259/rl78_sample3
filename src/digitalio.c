...
* Return Value : none
********************************************************************************/
void DGOUT_setValue( UCHAR port, UCHAR In, UCHAR Active )//30,1,low 
{
	UCHAR	ucVal;
//highでon→high
//highでoff→low
...
//lowでon→low
//lowでoff→high
	ucVal = In & 0x01;//ucval=1 //ucval=1
	if (Active==LOW){//low //high
		ucVal = (~ucVal) & 0x01;//1110 0001 =0
	}

	Port_Set(port/10,port%10,ucVal);//ucval=0 //ucval=1
}
...
UCHAR DGIN_getValue( UCHAR Port, UCHAR Value, UCHAR Trig )
{
	...
	if (ucInputValue != ucLastLet) {//portの値と入力値が一致していないとき

		if (Trig == 1) {
			/*  "Count value of DGIN" <- "Count value of DGIN" + 1 */
			uccount++;
			if ( uccount >= Value) {//カウントが1より大きいとき
